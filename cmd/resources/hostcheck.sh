...
# helper: get vendor:model for a PCI device in format "vendor:model" (e.g., "15b3:1021")
get_pci_vendor_model() {
    local pci="$1"
    [[ -z "$pci" ]] && echo "" && return
    ...
    local pci_path="/host-sys/bus/pci/devices/$pci"
    if [ -d "$pci_path" ]; then
        local vendor_file="$pci_path/vendor"
        local device_file="$pci_path/device"

        if [ -f "$vendor_file" ] && [ -f "$device_file" ]; then
            local vendor=$(cat "$vendor_file" 2>/dev/null | sed 's/0x//')
            local device=$(cat "$device_file" 2>/dev/null | sed 's/0x//')
...
# Helper function to get bond mode (sh-compatible alternative to associative arrays)
get_bond_mode() {
  local bond="$1"
  [ -f "/host-sys/class/net/$bond/bonding/mode" ] && cat "/host-sys/class/net/$bond/bonding/mode" 2>/dev/null || echo ""
}

# Helper function to get bond slaves (sh-compatible alternative to associative arrays)
get_bond_slaves() {
  local bond="$1"
...
for iface_dir in /host-sys/class/net/*; do
  ...
  bond_mode=""
  bond_slaves_json="[]"
  if [ -d "/host-sys/class/net/$ifname/bonding" ]; then
    # This is a bond interface
    is_bond=true
    bond_mode="$(get_bond_mode "$ifname")"

    # Build slaves JSON array
    slaves="$(get_bond_slaves "$ifname")"
    sjson=""
    sc=0
    for s in $slaves; do
      [ -n "$s" ] || continue
  ...
  vendor_model="$(get_pci_vendor_model "$pci_addr")"
  ...
  obj="$obj\"is_bond_slave\":$is_bond_slave,"

  # Add bond fields only for bonds
  if [ "$is_bond" = "true" ]; then
...
for nvme_dev in /host/dev/nvme[0-9]n[0-9]; do
  ...
  obj="$obj\"pci_address\":\"$(json_escape "$pci_addr")\","
  obj="$obj\"numa_node\":$numa_node,"
  obj="$obj\"vendor_model\":\"$(json_escape "$(get_pci_vendor_model "$pci_addr")")\","
