...
func withClients(ctx context.Context, clients *K8sClients) context.Context {
	return context.WithValue(ctx, ctxKeyClients, clients)
}

func getClients(ctx context.Context) *K8sClients {
	if clients, ok := ctx.Value(ctxKeyClients).(*K8sClients); ok {
		return clients
	}
	return nil
}
