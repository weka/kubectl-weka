...
func (c *CSIPodLogsCollector) collectPodLogs(ctx context.Context, baseDir string, pod *corev1.Pod, results chan<- podLogResult) {
	...
	currentLogs, _, err := collectPodLogs(ctx, KubeClients, pod.Namespace, pod.Name, "", &corev1.PodLogOptions{})
	if err != nil {
	...
	if metrics.RestartCount > 0 {
		previousLogs, _, err := collectPodLogs(ctx, KubeClients, pod.Namespace, pod.Name, "", &corev1.PodLogOptions{Previous: true})
		if err != nil {
