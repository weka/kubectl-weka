...
func RunHostChecks(ctx context.Context, clients *kubernetes.K8sClients, nodes []corev1.Node, opts HostCheckOptions) (HostChecksMap, error) {
	...
	needsCleanup := false
	if namespace == "" {
		namespace = fmt.Sprintf("kubectl-hostchk-%s", utils.RandomString(8))
		needsCleanup = true
	...
	cleanupFunc := func() {
		...
		if err := clients.CRClient.Delete(cleanupCtx, ns); err != nil {
			if opts.Verbose {
		...
		if !opts.CleanupInBackground {
			...
			for time.Now().Before(deleteDeadline) {
				...
				err := clients.CRClient.Get(cleanupCtx, ctrlclient.ObjectKey{Name: namespace}, &checkNs)
				if err != nil {
	...
	for time.Now().Before(deadline) {
		...
		for nodeName, pod := range createdPods {
			...
			if err := clients.CRClient.Get(ctx, ctrlclient.ObjectKey{
			...
			if currentPod.Status.Phase == corev1.PodSucceeded {
				...
				if _, exists := hostChecksMap[nodeName]; !exists {
					logs, err := kubernetes.GetPodLogs(ctx, clients, currentPod.Namespace, currentPod.Name, "hostchecks")
					if err != nil {
						...
						if err := json.Unmarshal([]byte(logs), &hc); err != nil {
							collectionErrors[nodeName] = fmt.Sprintf("failed to parse hostcheck JSON: %v\nFirst 200 chars of output: %s",
								err, utils.TruncateString(logs, 200))
							if opts.Verbose {
								fmt.Printf("  [%s] Failed to parse JSON: %v\n", nodeName, err)
								fmt.Printf("       Output: %s\n", utils.TruncateString(logs, 200))
							}
						} else {
...
func (hcm HostChecksMap) SaveToFile(filename string) error {
	...
	if err := utils.WriteFile(filename, string(data)); err != nil {
...
func (r *HostCheckModuleRegistry) GetHostChecksForNodes(
	...
	newResults, err := RunHostChecks(ctx, clients, uncachedNodes, opts)
	if err != nil {
...
func (r *HostCheckModuleRegistry) ValidateWithModules(
	...
	for nodeName, hostCheck := range hostChecksMap {
		...
		for _, moduleName := range config.ModuleNames {
			...
			resultStatus := types.StatusPass
			if err != nil {
				resultStatus = types.StatusFail
			}
...
func (r *HostCheckModuleRegistry) ValidateAll(
	...
	hostChecksMap, err := r.GetHostChecksForNodes(ctx, clients, nodes)
	if err != nil {
...
func (r *HostCheckModuleRegistry) FormatNodeValidationResults(
	...
	for _, mr := range moduleResults {
		if mr.Status == types.StatusFail {
			hasError = true
		} else if mr.Status == types.StatusWarn {
			hasWarning = true
		}
	}
	...
	for _, moduleName := range checkOrder {
		...
		displayText = utils.IndentText(displayText, 5, 3) // Indent module details for better readability
