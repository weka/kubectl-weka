...
func (nv *NetworkInterfaceValidation) setStatus(status types.CheckStatus) {
	switch status {
	case types.StatusFail:
		nv.Status = types.StatusFail
	case types.StatusWarn:
		if nv.Status != types.StatusFail {
			nv.Status = types.StatusWarn
		}
	case types.StatusPass:
		if nv.Status != types.StatusFail && nv.Status != types.StatusWarn {
			nv.Status = types.StatusPass
		}
	}
}
...
func (r *NetworkInterfacesModuleResponse) Status() types.CheckStatus          { return r.status }
...
func (m *NetworkInterfacesModule) Validate(podOutput string) (HostCheckModuleResponse, error) {
	...
	for _, iface := range candidates {
		...
		if validation.Status == types.StatusWarn {
			data.Warnings++
		}
	}
...
func (m *NetworkInterfacesModule) validateInterface(iface *NetworkInterface) *NetworkInterfaceValidation {
	...
	if validation.IPAddress == "" {
		...
		validation.setStatus(types.StatusFail)
	...
	if iface.MTU < 8000 && iface.IsEthernet() {
		validation.setStatus(types.StatusWarn)
	...
	validation.Reason = strings.Join(reasons, "\n")
	validation.setStatus(types.StatusPass) // will set only if empty
	return validation
}
...
func FormatNetworkInterfacesTable(data *NetworkInterfacesModuleData, visibleColumns []string) string {
	...
	// Define all columns
	allColumns := []printer.TableColumn{
	...
	var columns []printer.TableColumn
