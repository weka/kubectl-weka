...
func (r *NVMeDrivesModuleResponse) Status() types.CheckStatus { return r.status }
...
func (m *NVMeDrivesModule) Validate(podOutput string) (HostCheckModuleResponse, error) {
	...
	if err := json.Unmarshal([]byte(podOutput), &hc); err != nil {
		return &NVMeDrivesModuleResponse{status: types.StatusFail, moduleName: m.Name(), err: err}, err
	}
