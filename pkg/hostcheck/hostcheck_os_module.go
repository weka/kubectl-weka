...
func (r *OsModuleResponse) Status() types.CheckStatus { return r.status }
...
func (m *OSModule) Validate(podOutput string) (HostCheckModuleResponse, error) {
	...
	if err := json.Unmarshal([]byte(podOutput), &hc); err != nil {
		return &OsModuleResponse{status: types.StatusFail, moduleName: m.Name(), err: err}, err
	}
	...
	status := types.StatusFail // Default to fail unless we can confirm it's supported
