...
func (r *WekaAgentServiceModuleResponse) Status() types.CheckStatus { return r.status }
...
func (m *WekaAgentServiceModuleModule) Validate(podOutput string) (HostCheckModuleResponse, error) {
	...
	if err := json.Unmarshal([]byte(podOutput), &hc); err != nil {
		return &WekaAgentServiceModuleResponse{status: types.StatusFail, moduleName: m.Name(), err: err}, err
	}
