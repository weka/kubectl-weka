...
func collectorsByMode(mode types.CollectionMode, resourceName string) []Collector {
	...
	switch mode {
	case types.CollectionModeOperator:
		return []Collector{
			nodesCollector,
	...
	case types.CollectionModeK8s:
		return []Collector{
			nodesCollector,
			&K8sPreflightCollector{NodeSelector: supportBundleNodeSel},
		}

	case types.CollectionModeAll:
		return []Collector{
			nodesCollector,
			&OperatorLogsCollector{},
			&OperatorNodeAgentLogsCollector{},
...
func RunSupportBundleByMode(clients *kubernetes.K8sClients, mode types.CollectionMode, resourceName, namespace string, allNamespaces, includeSensitive bool) error {
...
func runSupportBundleCommand(ctx context.Context, clients *kubernetes.K8sClients, caseID, output, namespace string, allNamespaces bool, collectors []Collector) error {
	...
	if namespace == "" && !allNamespaces {
		namespace, err = kubernetes.GetKubeNamespace()
		if err != nil {
			return err
		}
	}
...
func CollectPodLogsParallel(ctx context.Context, clients *kubernetes.K8sClients, namespace string, pods []string, logsDir string, maxConcurrency int) ([]string, []string) {
	if len(pods) == 0 {
	...
	if len(nodeList.Items) > 0 {
		for _, node := range nodeList.Items {
			readyNodes[node.Name] = kubernetes.IsNodeReady(node)
		}
...
func CollectPodDescriptionsParallel(ctx context.Context, clients *kubernetes.K8sClients, namespace string, pods []string, descDir string, maxConcurrency int) ([]string, []string) {
	if len(pods) == 0 {
		return []string{}, []string{}
	}
